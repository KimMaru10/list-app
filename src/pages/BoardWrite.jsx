...
const BoardWrite = () => {
  ...
  const addBoardHandler = async () => {
    try {
      ...
      board.images.forEach(imageUrls => {
        formData.append('imageUrls', imageUrls); // 이미지 파일을 FormData에 추가
      });

      await addBoardList(formData); // FormData 객체를 전달하여 게시글 등록
      alert('등록되었습니다.');
      navigate('/board');
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      ...
      <div>
        <span>작성자</span>
        <input type="text" name="createdBy" value={board.createdBy} onChange={handleChange} />
      </div>
      <br />
      <div>
        <span>내용</span>
        <textarea name="content" cols="30" rows="10" value={board.content} onChange={handleChange}></textarea>
      </div>
      <div>
